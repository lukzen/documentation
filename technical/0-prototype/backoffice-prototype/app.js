...
const NAV_GROUPS = [
  ...
  {title:'Finance', items:[
    {id:'commission-policy',icon:'⚙',label:'Commission Policy'}
  ]}
];
...
function showScreen(id){
  ...
  renderSidebars(id);
  // Auto-collapse parent rail to icons on Commission Policy; restore everywhere else
  document.body.classList.toggle('cp-mode', id==='commission-policy');
  // Honor per-route pinned preference
  const pinned = id==='commission-policy' && localStorage.getItem('cp-rail-pinned')==='1';
  ...
  openModal(id?'Edit Sales Agent — '+a.name:'Add Sales Agent',
    `<div class="form-grid">
     ...
     <div class="modal-note">
       ...
       Commission percentages are managed in <a class="lnk" href="commission-config.html">Commission Policy →</a> as a layered policy (Tier default + optional per-hotel override). Editing rates here is intentionally disabled to maintain a single source of truth.
     </div>`,
    {onSave:()=>{
      const v = collect();
      if(a){
...
function openAgentDrawer(id, tab){
  ...
  const compHTML = `
    ...
    <div class="modal-note">
      Commission rates are managed centrally in <a class="lnk" href="commission-config.html?agent=${a.id}">Commission Policy →</a>.
  ...
  const earningsHTML = `
    ...
    <div class="modal-note">Earnings are computed by the Commission Policy engine for every booking this agent closed. Click any number for a layer breakdown (coming soon).</div>`;
...
/* ---------- Commission Policy embedded screen ---------- */
RENDERERS['commission-policy'] = () => {
