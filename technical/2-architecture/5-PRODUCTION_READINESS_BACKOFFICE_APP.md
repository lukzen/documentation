...
22. **Booking Breakdown 5-column finance table** — `src/modules/finance/BookingBreakdownPage.tsx` (/finance/booking-breakdown). Replaced 4 prior financial columns with 5 new ones in order: Ergos markup, Agency rebate, Sales agent earning, Agency markup, Customer price. Each renders "$X.XX (Y.Y%)"; cells without a frozen commissionSnapshot render '—'. Backed by new helpers in `backend-service/src/domains/backoffice/services/finance-row.service.ts`.
23. **Sortable tables across all 10 surfaces** — New shared primitive at `src/components/common/SortableTableHead.tsx` + `src/hooks/useTableSort.ts`. Click-to-sort chevron headers with aria-sort attribute; double-chevron (40% opacity) inactive, up=asc, down=desc. Two variants: pure SortableTableHead for raw-shadcn tables, SortableTableHeadTanStack for tables that already build a useReactTable instance. Migrated: BookingBreakdownPage, BookingAuditLogPage, ViewReservations, RolesManagementComponent, DashboardComponent, modification-history, GuestsInfoSection (2 tables), ListSalesAgentComponent, AgencyBookingsPnLPage, HotelCommissionOverridesTab. Server-paginated tables push sort to backend; client-paginated sort in memory.
24. **Backend finance list expansion + sort** — GET /admin/finance/bookings now returns per-row waterfall fields (ergosMarkupAmount/Pct, rebateAmount/Pct, agentEarningAmount/Pct) computed from the frozen commissionSnapshot via `computeMargin()`. Accepts sortBy/sortDir query params: DB-pushed for createdAt/bookingReference/status/hotelName; in-memory after row math for computed financial fields. Pure helpers extracted to `finance-row.service.ts` for unit-testability (no DB mocking).
25. **Agency P&L sort + end-of-day date filter** — GET /travel-agency/:id/pnl accepts sortBy/sortDir on all 10 visible columns; sort applied in-memory after per-row math; totals row unchanged regardless of sort. Both finance and P&L endpoints: "to" date filter now snaps to 23:59:59.999 UTC so same-day bookings created after midnight are included.
26. **Agency-app price consistency** — agencyCustomerPrice + agencyMarkupPctSnapshot now frozen on every booking (UnifiedBookingModel); backend DTO surfaces both; agency-app BookingPage threads agencyCustomerPrice through bookingDetails navigate state at 4 sites (single-room ± transfer; multi-room ± transfer); BookingConfirmationPage reads `agencyCustomerPrice ?? totalAmount`. Closes the prior $136.02 (pre-markup) vs $156.42 (post-markup) price-divergence between rate card, checkout, confirmation, and /bookings list.
27. **Tests** — 168 backoffice backend tests pass including new `finance-row.unit.test.ts` (sort + waterfall expansion for present/absent/zero-sell snapshots) and `travel-agency.pnl.unit.test.ts` (every column kind covered; totals-stable-regardless-of-sort regression guard). Agency-app added computeCustomerPrice tests in calculation.test.ts. Backend booking.dto.unit.test.ts pinned that agencyCustomerPrice + agencyMarkupPctSnapshot surface through the DTO mapper.
